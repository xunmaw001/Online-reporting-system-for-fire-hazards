	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback" :bottom="100">
		<view class="container">
					...
					<view class="text" :style='{"color":"rgba(0, 0, 0, 1)","textAlign":"left","fontSize":"28rpx"}'>{{detail.shijiandizhi}}</view>
                				                				...
                				                				<view class="detail-list-item" :style='{"boxShadow":"0 0 16rpx rgba(0,0,0,.3)","margin":"20rpx  20rpx 0px 0px","borderColor":"rgba(204, 153, 204, 1)","backgroundColor":"#fff","borderRadius":"20rpx","borderWidth":"2rpx","borderStyle":"solid","height":"68rpx"}'>
					<view class="lable" :style='{"width":"auto","fontSize":"28rpx","color":"rgba(204, 153, 204, 1)","textAlign":"left"}'>消防隐患分类：</view>
					<view class="text" :style='{"color":"rgba(0, 0, 0, 1)","textAlign":"left","fontSize":"28rpx"}'>{{detail.xiaofangyinhuanfenlei}}</view>
				</view>
...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','tongjifenlei');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','tongjifenlei');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('tongjifenlei', this.id);
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discusstongjifenlei/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('tongjifenlei', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('tongjifenlei', this.detail);
				this.$utils.msg('审核成功');
