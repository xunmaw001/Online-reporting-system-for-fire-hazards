          ...
          <el-form-item class="input" label="联系方式" prop="lianxifangshi">
              <el-input v-model="ruleForm.lianxifangshi" 
                placeholder="联系方式" readonly></el-input>
                                    ...
                                    <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="事件时间" prop="shijianshijian">
            <el-date-picker
                value-format="yyyy-MM-dd HH:mm:ss"
                v-model="ruleForm.shijianshijian" 
                type="datetime"
                placeholder="事件时间">
            </el-date-picker>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.shijianshijian" label="事件时间" prop="shijianshijian">
          ...
          <el-form-item class="input" label="事件地址" prop="shijiandizhi">
              <el-input v-model="ruleForm.shijiandizhi" 
                placeholder="事件地址" readonly></el-input>
                                                                                ...
                                                                                <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="事件描述" prop="shijianmiaoshu">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="事件描述"
                  v-model="ruleForm.shijianmiaoshu" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.shijianmiaoshu" label="事件描述" prop="shijianmiaoshu">
                    <span>{{ruleForm.shijianmiaoshu}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
                                                                        ...
                                                                        // 多级联动参数
                                                                                            info(id) {
      this.$http({
        url: `xiaofangyinhuanjubao/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `xiaofangyinhuanjubao/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.xiaofangyinhuanjubaoCrossAddOrUpdateFlag = false;
                  this.parent.search();
